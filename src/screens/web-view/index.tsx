...
export default function WebViewScreen(): React.JSX.Element {
  ...
  // UHF RFID State
  const [uhfPower, setUhfPower] = useState(12);
  ...
  const handleNFCResult = useCallback((result: NFCTagResult): void => {
    ...
    if (result.success && (result.id || result.idHex)) {
      ...
      // Always dispatch event to WebView (like UHF)
      const script = `
        window.dispatchEvent(new CustomEvent('nfc-tag-read', { detail: ${JSON.stringify(
          nfcData,
        )} }));
        true;
      `;
  ...
  // Start NFC Scan
  const startNFCScan = useCallback(async (): Promise<void> => {
    if (!isNFCInitialized) {
    ...
    <View style={styles.container}>
      ...
      >
        <WebView
          ...
          originWhitelist={['*']}
          source={{
            uri: 'https://jharkhandegovernance.com/mobile',
          }}
          ...
          onMessage={async (event: WebViewMessageEvent) => {
            ...
            if (message?.Key === 'RELOAD') {
              ...
              console.log('WebView: Received START_NFC_SCAN message');
              startNFCScan();
        ...
        {loading ? (
          ...
          >
            ...
            <Text style={{ marginTop: 75, fontSize: 16, fontWeight: 'bold' }}>
              {isNFCScanning
                ? // with
                  `Scanning for NFC tags... `
                : isUHFScanning
                ? 'Scanning for UHF RFID tags...'
                : 'Loading'}
            </Text>
